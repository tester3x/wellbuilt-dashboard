...
interface Props {
  ...
  /** Whether the current user may edit required photo specs (manageCompany). */
  canEdit: boolean;
}
 ...
 */
export function RequiredPhotoSpecs({ company, canEdit }: Props) {
  ...
  const addReq = () => { if (!canEdit) return; setReqs(rs => [...rs, blankReq()]); setDirty(true); setSavedMsg(''); };
  const removeReq = (id: string) => { if (!canEdit) return; setReqs(rs => rs.filter(r => r.id !== id)); setDirty(true); setSavedMsg(''); };
  ...
  return (
    <div className="border-t border-gray-700 pt-3 mt-1">
      ...
      {!canEdit && (
        <div className="text-gray-400 text-xs mb-1">View-only — you do not have permission to change required photo specs.</div>
      )}
      ...
      {operators.length === 0 ? (
        ...
        <>
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <button
              onClick={() => { if (!canEdit) return; setEnabled(v => !v); setDirty(true); setSavedMsg(''); }}
              disabled={!canEdit}
              className={`relative w-10 h-5 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${enabled ? 'bg-orange-500' : 'bg-gray-600'}`}
            >
            ...
            <div className="space-y-3 mt-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-shrink-0">
                      <label className="cursor-pointer block">
                        ...
                        <input type="file" accept="image/*" className="hidden" disabled={!canEdit}
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`relative w-9 h-5 rounded-full transition-colors flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed ${r.active ? 'bg-green-600' : 'bg-gray-600'}`}
                        >
                        ...
                        {defaultRequirementById(r.id) && (
                          <button
                            ...
                            disabled={!canEdit}
                            className="text-blue-300/70 hover:text-blue-200 text-xs flex-shrink-0 px-1 disabled:opacity-50 disabled:cursor-not-allowed"
                        ...
                        <button
                          ...
                          disabled={!canEdit}
                          className="text-red-400/70 hover:text-red-300 text-sm flex-shrink-0 px-1 disabled:opacity-50 disabled:cursor-not-allowed"
                      ...
                      <textarea
                        ...
                        rows={6}
                        className="w-full px-2 py-1.5 bg-gray-700 border border-gray-600 rounded text-white text-xs resize-y min-h-[7rem] leading-relaxed disabled:opacity-50"
                      />
                      ...
                      {(() => {
                        ...
                        const suggestDisabled = suggesting || !hasSample || !canEdit;
                        return (
                          <div className="flex items-center gap-2">
                            <input
                              value={hintById[r.id] || ''}
                              ...
                              className="flex-1 px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white text-xs placeholder-gray-500 disabled:opacity-50"
                            />
                            <button
                              onClick={() => onSuggest(r)}
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        <label className="flex items-center gap-1 text-gray-400 text-xs">
                          ...
                          <input type="number" min={1} max={10} value={r.requiredCount}
                            ...
                            disabled={!canEdit}
                            className="w-12 px-1 py-0.5 bg-gray-700 border border-gray-600 rounded text-white text-xs text-center disabled:opacity-50" />
                        </label>
                        <label className="flex items-center gap-1 text-gray-400 text-xs">
                          Phase
                          <select value={r.phase}
                            ...
                            disabled={!canEdit}
                            className="px-1 py-0.5 bg-gray-700 border border-gray-600 rounded text-white text-xs disabled:opacity-50">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <button onClick={addReq} disabled={!canEdit} className="text-orange-400 hover:text-orange-300 text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed">+ Add Required Photo</button>
                  <button onClick={resetAll} disabled={!canEdit} className="text-blue-300 hover:text-blue-200 text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed" title="Replace matching WB-default requirements; keep custom ones; re-add missing defaults">↺ Reset All to WB Defaults</button>
                </div>

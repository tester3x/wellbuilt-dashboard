...
export const SESSION_AUDIENCE_WBT = 'wbt';
 ...
 * legitimately WB-T.
 */
export const SESSION_APP_CLAIM = 'app';
export function isSessionAudience(v) {
    return typeof v === 'string' && SESSION_AUDIENCES.includes(v);
}
/**
 ...
 */
export function readSessionAudience(request) {
    if (typeof request !== 'object' || request === null || Array.isArray(request)) {
        return { kind: 'absent' };
    }
    const raw = request.audience;
    if (raw === undefined || raw === null)
        return { kind: 'absent' };
    if (!isSessionAudience(raw))
        return { kind: 'rejected', reason: 'unsupported_audience' };
 ...
 */
export const CLIENT_FORBIDDEN_CLAIM_KEYS = Object.freeze([
    'app',
    'claims',
    'customClaims',
    'developerClaims',
...
export function containsClientClaimMaterial(request) {
    if (typeof request !== 'object' || request === null || Array.isArray(request))
        return false;
 ...
 */
export function sessionClaimsForAudience(outcome) {
    if (outcome.kind !== 'audience')
        return Object.freeze({});
    return Object.freeze({ [SESSION_APP_CLAIM]: outcome.audience });
}
//# sourceMappingURL=sessionAudience.js.map
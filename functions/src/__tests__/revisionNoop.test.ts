...
describe('processEditRequest wiring (structural)', () => {
  ...
  test('applied revisions may stamp lastRevisionAt (audit) — both apply paths', () => {
    ...
    // applied dateTimeUTC comes only from the edit or the stored original.
    expect(fn).toContain('const newDateTimeUTC = data.dateTimeUTC || origPacket.dateTimeUTC;');
  });
  ...
  test('stale revisions acknowledge and drop without any write', () => {
    ...
    const block = fn.slice(staleIdx, staleIdx + 500);

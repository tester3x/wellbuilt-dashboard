...
describe('evaluateWbtWellLookup', () => {
  ...
  it('prefers API number then falls back to canonical name; fail-closed on miss/stale/ambiguous/wrong company', () => {
    ...
    const byApi = evaluateWbtWellLookup({
      wellId: '33-053-09031-00-00',
      wellName: 'GABRIEL 1-36-25H',
      companyId: COMPANY,
      wellConfig: mixed,
    });
    expect(byApi.ok).toBe(true);
    if (byApi.ok) expect(byApi.wellConfigKey).toBe('Gabriel 1');

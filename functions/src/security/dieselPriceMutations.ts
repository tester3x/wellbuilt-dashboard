...
/**
 * Loads the caller's profile with defense-in-depth:
 * 1. Checks RTDB `users/{uid}` for companyId, roles, and company roleCapabilities overrides.
 * 2. Falls back to token claims (for emulators and token-based callers).
 */
...
export const staffSaveDieselPrice = httpsV2.onCall(OPTS, async (request) => {
  ...
  // Validate price, date, source (including server-side date ceiling)
  ...
  if (!val.ok) {
    if (val.reason === 'future_date_rejected') {
      throw new httpsV2.HttpsError(
        'invalid-argument',
        'future_date_rejected: Manual fuel price date cannot be in the future'
      );
    }

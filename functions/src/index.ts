...
import { resolveAuthoritativeWellConfig } from './security/resolveAuthoritativeWellConfig';
  ...
  .onCreate(async (snapshot, context) => {
    ...
    if (outgoingSnap.exists()) {
      ...
      legacySnap.forEach((child) => {
        ...
        const matchesExactIdentity = Boolean(
          rowCompany && rowCompany === boundCompanyId && rowWellId && rowWellId === boundWellId
        );
        if (matchesExactIdentity) {
    ...
    if (!holdsHighWater) {
      ...
      oldResponses.forEach((child) => {
        ...
        const rowCompany = typeof row.companyId === 'string' ? row.companyId.trim() : '';
        const rowWellId = canonicalWellId(row);
        const isSameCompany = Boolean(rowCompany && rowCompany === boundCompanyId);
        const isSameWell = Boolean(rowWellId && rowWellId === boundWellId);

...
/** Validate a manual Save's bounded input. Server-side sanitization and date ceiling. */
export function validateManualPrice(
  ...
  // Date ceiling: manual date cannot exceed server's current calendar date (UTC)
  const nowDate = now instanceof Date ? now : new Date(now);
  const serverToday = nowDate.toISOString().slice(0, 10);
  if (date > serverToday) {
    return { ok: false, reason: 'future_date_rejected' };
  }

...
const DEFAULT_ROLE_CAPABILITIES: Record<string, string[]> = {
  it: ['manageEquipment', 'viewAllCompanies'],
...
/** Dashboard user with manageEquipment capability and tenant scope. */
export async function requireDashboardEquipmentManager(
  ...
  if (!hasCapability(roles, 'manageEquipment', overrides)) {
    throw new httpsV2.HttpsError('permission-denied', 'manageEquipment capability required');
  }

  const isPlatformAdmin = !userCompanyId && roles.some((r) => r === 'admin' || r === 'it');
  if (!isPlatformAdmin && userCompanyId !== targetCompanyId) {
    throw new httpsV2.HttpsError('permission-denied', 'Cannot manage equipment for another company');
  }

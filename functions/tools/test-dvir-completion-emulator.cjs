...
(async () => {
  ...
  await denied(api.resolveDriverDvirRecovery.run({ data: owner }), 'unauthenticated');
  await denied(api.resolveDriverDvirRecovery.run(request(owner, 'driver-b', 'suite')), 'permission-denied');
  const recovery = await api.resolveDriverDvirRecovery.run(request(owner, 'driver-a', 'suite'));
  assert.deepEqual(recovery.recovery, { shiftId: old, phase: 'post_trip' });
  await api.recordDriverDvirRecoveryFeedback.run(request({ ...owner, shiftId: old, reason: 'app_or_connection', note: 'Test connection loss' }, 'driver-a', 'suite'));
  await denied(api.recordDriverDvirRecoveryFeedback.run(request({ ...owner, shiftId: old, reason: 'invalid' })), 'invalid-argument');
  await denied(api.recordDriverDvirRecoveryFeedback.run(request({ ...owner, shiftId: old, reason: 'forgot' }, 'driver-b')), 'permission-denied');
  ...
  assert.equal((await api.resolveDriverDvirRecovery.run(request(owner))).recovery, null);
  await denied(api.recordDriverDvirRecoveryFeedback.run(request({ ...owner, shiftId: old, reason: 'forgot' })), 'failed-precondition');
  ...
  assert.equal((await api.resolveDriverDvirRecovery.run(request(owner))).recovery, null, 'current open shift is not recovery');
  await fs.doc('driver_shift_authority/driver-a').update({ openPeriodId: null });
  assert.deepEqual((await api.resolveDriverDvirRecovery.run(request(owner))).recovery, { shiftId: current, phase: 'post_trip' });
  const { buildSsoDeps } = require('../lib/sso/ssoCallables');
  const deps = buildSsoDeps();
  assert.equal(await deps.canRecoverDvir('driver-a', 'company-a', current), true);
  assert.equal(await deps.canRecoverDvir('driver-b', 'company-a', current), false);
  assert.equal(await deps.canRecoverDvir('driver-a', 'foreign', current), false);
  assert.equal(await deps.canRecoverDvir('driver-a', 'company-a', old), false);
  assert.equal((await fs.doc('driver_shift_authority/driver-a').get()).data().openPeriodId, null);
  await fs.doc('driver_shift_authority/driver-a').set(authorityBefore);

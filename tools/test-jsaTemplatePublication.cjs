...
(async()=>{
 await api.saveJsaTemplate('company','load',{name:'Loading',tasks:['loading'],version:99,status:'active',updatedBy:'forged'},'forged');
 assert.deepEqual(calls[0],{companyId:'company',templateId:'load',operation:'save',data:{name:'Loading',tasks:['loading']}});
